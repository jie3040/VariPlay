...
def extract_boxed(text):
    """Extract all \boxed{...} spans, including nested braces."""
    results, i = [], 0
    prefix = r'\boxed{'
    plen = len(prefix)
...
def get_response_mask(response_ids, eos_token_id, dtype):
    """Build a token mask until EOS. Kept here for compatibility/debugging."""
...
def main(args):
    """Generate one json shard of curriculum questions with a frozen model."""
    ...
    # Fixed question-setter prompt. The curriculum model must output exactly
    # a <question> block and a boxed answer so later stages can parse it.
    chat = [
        {
            "role": "system",
            "content": (
    ...
    # Generate many independent problems from the same prompt.
    ...
    for completion in completions:
        ...
        try:
            # Keep only parseable question/answer pairs. score=0 means "ready
            # for later executor evaluation"; score=-1 marks malformed output.
...
if __name__ == "__main__":
    # question_generate.bash supplies one suffix per GPU/shard.
