...
def _metric_safe_score(score: Dict) -> Dict[str, float]:
    """把返回给 verl reducer �?score 清洗成纯数值�?
    JSONL 记录仍保留完�?reward_breakdown；这里仅处理训练日志 metric�?    避免 None/dict �?np.mean 聚合时报错�?    """
    safe = {}
...
def compute_score(
    ...
    for i in tqdm(range(len(final_results)), desc=" - Calculating final scores"):
        ...
        record_score = score
        scores.append(_metric_safe_score(record_score))
        writer.add_row(_build_record(
            ...
            critic_scores=critic_score_rows[i],
            verifier_writable=record_score.get("verifier_writable"),
            model_valid_rate=record_score.get("model_valid_rate"),
            model_mean_adv_reject_rate=record_score.get("model_mean_adv_reject_rate"),
            reward_weights=record_score.get("weights"),
