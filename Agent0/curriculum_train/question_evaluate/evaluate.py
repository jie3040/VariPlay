...
# Input comes from question_generate.py; output is consumed by upload.py.
...
# score=0 means "valid generated question that still needs executor scoring".
...
# For each generated question, sample multiple executor answers. The majority
# agreement ratio becomes the self-consistency score.
...
# This evaluator is single-turn text reasoning. Curriculum training itself uses
# start_vllm_server_tool.py for multi-turn tool-augmented scoring.
...
for response, golden_answer, question in zip(responses, answers, questions):
    try:
        ...
        # Keep the majority answer and its self-consistency score. upload.py
        # later filters for scores in a middle band such as [0.3, 0.8].
        results_all.append({
            "question": question,
