...
def expand_seeds(n_questions: int):
    """把固�?seed 扩展�?F.2 需要的 question 数量�?
    �?8 条保�?Phase 4 spec 原样；超�?8 条时生成确定�?variant�?    只改变题面措辞，不改�?expected_answer，确�?smoke 可复现�?    """
    seeds = []
    for idx in range(n_questions):
        base = MULTIQ_SEEDS[idx % len(MULTIQ_SEEDS)]
        if idx < len(MULTIQ_SEEDS):
            seeds.append(base)
            continue
        round_id = idx // len(MULTIQ_SEEDS)
        seeds.append({
            **base,
            "question": (
                f"{base['question']} "
                f"Variant {round_id}: solve independently and output only the final answer."
            ),
            "difficulty": f"{base['difficulty']}_variant",
        })
    return seeds


def build_batch(n_questions: int, n_traj_per_question: int):
    """�?seed 展开�?question × trajectory 的固�?smoke batch�?""
    seeds = expand_seeds(n_questions)
    predicts = []
    ground_truths = []
    prompt_texts = []
    prompt_messages = []
