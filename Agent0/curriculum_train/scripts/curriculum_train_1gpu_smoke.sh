...
CUDA_VISIBLE_DEVICES=0 python3 -m verl.trainer.main \
  ...
  worker.rollout.gpu_memory_utilization="${CURRICULUM_ROLLOUT_GPU_UTIL}" \
  ...
  trainer.val_freq=-1 \
  trainer.save_freq="${CURRICULUM_SAVE_FREQ}" \
  trainer.save_limit=2 \
  trainer.total_epochs="${CURRICULUM_STEPS}" \
  trainer.max_steps="${CURRICULUM_STEPS}"
