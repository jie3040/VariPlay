...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, path: str):
        ...
        if os.getenv("VERIPLAY_LIGHT_CKPT", "0") == "1" or os.getenv("CRITIC_SMOKE_LIGHT_CKPT", "0") == "1":
            # Phase 4 smoke 在单�?AutoDL 环境中只需要模型落盘作�?stage artifact�?            # 跳过 optimizer state 可避�?Ray �?CPU RAM 上序列化大对象时 OOM�?            model_state_dict = self.model.state_dict()
            extra_state_dict = {
            ...
            model_path = os.path.join(path, f"model_world_size_{self.world_size}_rank_{self.rank}.pt")
            extra_path = os.path.join(path, f"extra_state_world_size_{self.world_size}_rank_{self.rank}.pt")
            print(f"[rank-{self.rank}]: Saving lightweight model to {os.path.abspath(model_path)}.")
            print(f"[rank-{self.rank}]: Saving lightweight extra_state to {os.path.abspath(extra_path)}.")
