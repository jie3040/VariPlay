...
# Train the executor agent with ADPO and tool-integrated rollouts.
# This script assumes a single 8-GPU node by default.

# Dataset generated by Agent0 data curation or a cold-start DeepMath parquet.
...
# Base or previous-iteration executor checkpoint.
model_name=Qwen/Qwen3-4B-Base # Qwen/Qwen3-8B-Base

# ADPO is registered in verl_tool/trainer/ppo/core_algos.py.
rl_alg=adpo
n_gpus_per_node=8
n_nodes=1

# Number of rollout trajectories sampled per prompt.
n=16
batch_size=128
...
# Local python-code tool server. During rollout, model actions are sent here and
# observations are appended back into the conversation.
python -m verl_tool.servers.serve --host $host --port $port --tool_type "python_code" --workers_per_tool 8 &
server_pid=$!

echo "Server (pid=$server_pid) started at $tool_server_url"

# Main VeRL-Tool PPO/GRPO/ADPO entrypoint. Most CLI args override Hydra config
# values under verl_tool/trainer/config.
...
# Stop the tool server after training exits.
