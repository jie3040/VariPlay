...
def _check_file(py_file: pathlib.Path, project_root: pathlib.Path, allow_list: list[str]) -> list[str]:
    ...
    for node in ast.walk(tree):
        ...
        for alias in node.names:
            ...
            except Exception:  # pragma: no cover ‚Ä?wide net for import quirks
                pass
...
def main() -> None:
    ...
    if not target_path.is_file():
        raise Exception(f"‚ù?Target file not found: {target_path}")
    ...
    if errors:
        ...
        raise Exception("‚ù?Docstring verification failed.")

    if not args.quiet:
        print(f"‚ú?All explicitly imported functions/classes in {target_path} have docstrings.")
