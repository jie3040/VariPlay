...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check that test files follow tests/<module>/‚Ä?layout.")
    parser.add_argument(
        "--impl-root",
        type=Path,
        default="verl",
    ...
    if violations:
        print("‚ù? Test layout violations found:\n", file=sys.stderr)
        for err in violations:
            print("  -", err, file=sys.stderr)

        print(
            f"\nGuideline:\n  Place each test file under   tests/<module_name>/‚Ä¶\n  where <module_name> is "
            f"one of the top-level packages inside '{args.impl_root}', or is explicitly listed via --allow-dirs.\n",
            file=sys.stderr,
        )
        raise Exception("‚ù? Test layout violations found.")

    print("‚ú? Tests folder structure looks good.")
