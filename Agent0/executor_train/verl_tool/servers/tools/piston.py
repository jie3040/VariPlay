...
class PistonTool(BaseTool):
    ...
    def __init__(self, num_workers=1, api_url=None, use_local=False):
        ...
        try:
            ...
            if self.is_public_api:
                logger.info("‚ú?Connected to Piston public API")
            else:
                logger.info("‚ú?Connected to local Piston API")
    ...
    def _show_docker_guide(self):
        ...
        guide = """
‚ù?Piston Docker container is not running or API is not accessible
