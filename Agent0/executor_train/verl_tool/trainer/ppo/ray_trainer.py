            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if "score" in batch.non_tensor_batch:
                        # 某些 parquet / datasets 嵌套字段会在 repeat/union 后变�?object 数组�?                        # 这里在最终入 tensor 前强制转�?float，保�?ADPO �?score 可以参与计算�?                        scores = np.asarray(batch.non_tensor_batch["score"], dtype=np.float32)
                        batch.batch["score"] = torch.tensor(
                            scores,
