...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        for _ in tqdm(range(self.config.trainer.total_epochs), desc="Epoch", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with timer("step", timing_raw):
                    ...
                    group_by = getattr(self.config.algorithm, "group_by", "uid")
                    if group_by != "uid":
                        if group_by not in batch.non_tensor_batch:
                            raise KeyError(f"algorithm.group_by={group_by} not found in non_tensor_batch")
                        # Critic GRPO 需要同一 question �?verifier 共享 group_id�?                        # 这里复用 uid 字段进入现有 advantage 计算路径�?                        batch.non_tensor_batch["uid"] = np.array(
                            [str(item) for item in batch.non_tensor_batch[group_by]], dtype=object
                        )
                    else:
                    ...
                    if os.getenv("GROUP_BY_SANITY_DEBUG", "0") == "1":
                        group_ids = batch.non_tensor_batch.get("group_id", None)
                        if group_ids is not None:
                            from collections import Counter
                            print(f"[group_by sanity] groups in batch: {dict(Counter(group_ids))}")
                            uid_ids = batch.non_tensor_batch.get("uid", None)
                            if uid_ids is not None:
                                print(f"[group_by sanity] uid groups in batch: {dict(Counter(uid_ids))}")

                    # compute global_valid tokens
                    batch.meta_info["global_token_num"] = torch.sum(batch.batch["attention_mask"], dim=-1).tolist()
                    # Phase 1 可观测性：把当前训练步传给 reward worker�?                    # 用于�?step_000001.jsonl 这样的文件名保存结构化记录�?                    batch.meta_info["global_step"] = self.global_step
        ...
        # val_freq < 0 表示禁用验证；smoke run 用它避免训练后再跑一次验证�?        if self.val_reward_fn is not None and self.config.trainer.val_freq >= 0:
            if (
                val_metrics is None
        ...
        # save_freq < 0 表示禁用保存；smoke run 不需要写完整 checkpoint�?        if self.config.trainer.save_freq > 0 and self.global_step % self.config.trainer.save_freq != 0:
            self._save_checkpoint()
