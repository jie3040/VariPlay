...
# smoke run 直接�?curriculum_train 源码目录运行；models 是顶层包而不�?verl 子包�?from models.transformers.qwen2_vl import get_rope_index
...
class RLHFDataset(Dataset):
    ...
    def _build_messages(self, example: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if "critic_format" in self.format_prompt:
            question = str(example.get("question", prompt_str))
            return [
                {
                    "role": "system",
                    "content": CRITIC_SYSTEM_PROMPT.replace("{TASK_DESCRIPTION}", question),
                },
                {
                    "role": "user",
                    "content": CRITIC_USER_TEMPLATE,
                },
            ]
        if "questioner_format_with_persona" in self.format_prompt:
            print("load personas")
    ...
    def __getitem__(self, index):
        ...
        # Phase 1.5 可观测性：raw_prompt_ids 会被 rollout worker 消费�?pop 掉，
        # 因此额外保存文本�?prompt 和原�?chat messages，供 reward 记录落盘�?        example["prompt_text"] = prompt
        example["prompt_messages"] = messages
