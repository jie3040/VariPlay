...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, path: str):
        ...
        if os.getenv("CRITIC_SMOKE_LIGHT_CKPT", "0") == "1":
            # Phase 4 Step E 只验�?plumbing。完�?optimizer checkpoint �?62GB
            # CPU RAM �?AutoDL smoke 环境里会触发 Ray OOM，因此这里提�?            # model-only 轻量落盘路径；正式训练不设置该环境变量�?            model_state_dict = self.model.state_dict()
            extra_state_dict = {
